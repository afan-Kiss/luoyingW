...
public class KeepLiveService extends Service implements IEventListener {
    private static XHCustomConfig customConfig;
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent == null || intent.getStringExtra("Username") == null){
            return super.onStartCommand(null, 0, 0) ;
        }

        initSDK(intent.getStringExtra("Username"));
        return super.onStartCommand(intent, flags, startId);
    }

    private void initSDK(String username) {
        MLOC.init(this, username);
        initFree();
    }

    private boolean isLogin = false;

    public static void setVoipServerUrl(String url){
        customConfig.setVoipServerUrl(url);
    }
